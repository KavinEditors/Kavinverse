...
(function typing() {
  ...
  function typeChar() {
    if (i <= text.length) {
      ...
      // keep cursor blinking - no deletion
...
/* --------------------- Skill tree: position icons + animate white pulse --------------------- */
(function(){
  ...
  function svgPointToScreen(pt) {
    ...
    return {
      x: rect.left + (pt.x - vb.x) * scaleX,
      y: rect.top + (pt.y - vb.y) * scaleY
    };
  }
  ...
  // build route: move along base left->right with short up/down branch excursions
  function buildPulseRoute() {
    ...
    // add gentle excursions up some branches for effect
...
(function(){
  ...
  async function loadStats(){
    try {
      ...
      // approximate commits (contributors): may be rate-limited and approximate
      let commitSum = 0;
      for (let r of repos){
        try {
